#!/usr/bin/env python
"""GitHub Copilot PR-review helper for the `implement-feature` skill.

After the skill opens a draft PR carrying the feature implementation and
requests a review from GitHub Copilot (the `copilot-pull-request-reviewer[bot]`),
Copilot reviews asynchronously -- usually within a few minutes, sometimes
longer. This helper does the fiddly, deterministic parts so the agent doesn't
shell out control-flow of its own (which would trigger extra permission
prompts):
  ...
  resolve <PR#> <comment-id> [<comment-id> ...] -- mark PR review comments
                   as resolved and reply with "Done". Only resolves comments
                   that are not already resolved.
...
already uses to add Copilot in the first place.
  ...
  if_pr_review.py wait <PR#> [--repo-slug owner/repo] [--dir DIR]
       [--timeout 480] [--poll 30] [--after-review-id N] [--proxy URL]
  if_pr_review.py rerequest <PR#> [--repo-slug owner/repo] [--proxy URL]
  if_pr_review.py resolve <PR#> <comment-id> [<comment-id> ...]
...
# The VTune build repo. Resolution order (matches if_bootstrap.py): explicit
...
def cmd_resolve(args):
    """Resolve PR review comments by replying 'Done' and marking them resolved.

    Only acts on comments that are not already resolved. Uses the GraphQL API
    because the REST API doesn't expose resolution state reliably.
    """
    slug = _resolve_repo_slug(args.repo, args.repo_slug, args.proxy)
    pr = _resolve_pr(args.repo, args.pr, slug, args.proxy)
    if pr is None:
    ...
    for cid in comment_ids:
        # First check if the comment is already resolved via GraphQL
        # We need the comment's node_id for GraphQL operations
        rc, out, err = _gh(
            ["api", "repos/{0}/pulls/comments/{1}".format(slug, cid)],
            proxy=args.proxy)
        if rc != 0:
            failed.append((cid, "fetch failed: {0}".format(err.strip() or out.strip())))
            continue

        try:
            comment_data = json.loads(out) if out.strip() else {}
            node_id = comment_data.get("node_id")
            if not node_id:
                failed.append((cid, "no node_id in comment data"))
                continue
        except ValueError as exc:
            failed.append((cid, "JSON parse failed: {0}".format(exc)))
            continue

        # Check resolution state via GraphQL
        query = """
        query($nodeId: ID!) {
          node(id: $nodeId) {
        ...
        """
        rc, out, err = _gh(
            ["api", "graphql", "-f", "query={0}".format(query),
             "-f", "nodeId={0}".format(node_id)],
            proxy=args.proxy)
        if rc != 0:
            failed.append((cid, "GraphQL check failed: {0}".format(err.strip() or out.strip())))
            continue
        ...
        try:
            ...
            failed.append((cid, "parse GraphQL response failed: {0}".format(exc)))
            continue
        ...
        # Resolve the comment via GraphQL
        mutation = """
...
def main(argv=None):
    ...
    s = sub.add_parser("resolve", help="resolve PR review comments by replying 'Done'")
    s.add_argument("pr", nargs="?", help="PR number (default: PR of current branch)")
    s.add_argument("comment_ids", nargs="+", help="comment IDs to resolve")
